...
export interface UserProgress {
  items: { [key: string]: { stars: number; bestScore: number; isLearned?: boolean; attempts?: number; firstAttempted?: string; lastAttempted?: string } };
  badges: string[]; // e.g., "animals_easy"
  totalStars: number;
  totalAttempts: number;
  streak: number;          // Real consecutive-day streak
  lastUsedDate: string;    // ISO date string (date only, e.g. "2026-05-03")
}
...
export class SpeechAssessmentEngine {
  ...
  async getClinicalReport(): Promise<ClinicalReport> {
    try {
      ...
      // I2 FIX: Use the composite score (10-100 scale) for overallAccuracy,
      // not phoneticAccuracy which can be 0 on recognition errors.
      const avgScore = history.length > 0
        ? history.reduce((acc, h) => acc + h.score, 0) / history.length
        : 0;

      // Separate phonetic accuracy for strength/weakness analysis (still useful internally)
      const avgPhonetic = history.length > 0
        ? history.reduce((acc, h) => acc + (h.phoneticAccuracy || 0), 0) / history.length
        : 0;
      ...
      return {
        overallAccuracy: Math.floor(avgScore), // I2 FIX: composite score, not phonetic
        ...
        clinicalNote: this.generateClinicalNote(avgScore, last10),
  ...
  private generateClinicalNote(avgAcc: number, trend: number[]): string {
    ...
    // C4 FIX: Use ±10 threshold on 10-100 scale for clinically meaningful trend detection.
    // A diff of ±5 is within normal session-to-session noise; ±10 signals real change.
    let note = `The user shows an average overall accuracy of ${Math.floor(avgAcc || 0)}%. `;
    if (diff > 10) note += "A clear positive trend in clarity is observed over recent attempts.";
    else if (diff < -10) note += "Some decline in consistency noted; may indicate fatigue or increased difficulty.";
  ...
  async getProgress(): Promise<UserProgress> {
    try {
      ...
      if (!exists) return { items: {}, badges: [], totalStars: 0, totalAttempts: 0, streak: 0, lastUsedDate: '' };
      const content = await RNFS.readFile(this.progressPath);
      const parsed = JSON.parse(content);
      // Backfill streak/lastUsedDate for existing saves that predate this field
      if (parsed.streak === undefined) parsed.streak = 0;
      if (!parsed.lastUsedDate) parsed.lastUsedDate = '';
      return parsed;
    } catch (e) {
      return { items: {}, badges: [], totalStars: 0, totalAttempts: 0, streak: 0, lastUsedDate: '' };
    }
  }
  ...
  async recordLearning(word: string) {
    try {
      ...
      const currentItem = progress.items[wordKey] || {
        stars: 0,
        bestScore: 0,
        isLearned: false,
        attempts: 0,
  ...
  async getAnalytics(): Promise<overallAnalytics> {
    try {
      ...
      return {
        ...
        totalAvailable,
        averageAccuracy,
        streak: progress.streak || 0, // I1 FIX: real streak from persisted progress
  ...
  private async updateProgress(result: PronunciationResult, category?: string, difficulty?: string) {
    try {
      ...
      // I1 FIX: Real streak tracking — compare today's date to lastUsedDate
      const todayStr = new Date().toISOString().slice(0, 10); // "YYYY-MM-DD"
      if (progress.lastUsedDate !== todayStr) {
        const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
        progress.streak = progress.lastUsedDate === yesterday
          ? (progress.streak || 0) + 1
          : 1; // Reset streak if gap > 1 day
        progress.lastUsedDate = todayStr;
      }

      // Calculate star (Binary: 1 star only if score >= 95)
      const hasStar = result.score >= 95 ? 1 : 0;
      ...
      currentItem.firstAttempted = currentItem.firstAttempted || now; // C5: timestamp
      currentItem.lastAttempted = now;                                 // C5: timestamp
      ...
      if (category && difficulty) {
        ...
        if (!progress.badges.includes(badgeId)) {
          // C4 (N4 note): vocab required once per call, cached by Metro bundler
          const vocab = require('../../assets/ar/vocabulary-data.json');
          const catData = vocab.categories.find((c: any) => c.id.toLowerCase() === category.toLowerCase());
          const targetItems = catData ? catData.items.filter((i: any) => i.difficulty === difficulty.toLowerCase()) : [];

          // C2 FIX: bestScore >= 90 (not === 100) — a perfect STT result gives ~95, not 100
